...
class TeleTreceArticleScraper {
  ...
  private parseContent(html: string, url: string): ScrapedArticleDetail {
    ...
    const publishedDate = mainArticle.find('time[itemprop="datePublished"]').attr('datetime');
    ...
    contentElement.find('.ads13, .leetambien, .embed, #t13-envivo, #article-inread-desktop, .banner-google-news, #banner-13go, .articulo-categorias, .cuerpo-share').remove();
  ...
  public async scrapeArticle(url: string): Promise<ScrapedArticleDetail | null> {
