...
class ElMostradorArticleScraper {
  private async fetchArticleHtml(url: string): Promise<string | null> {
    try {
      ...
      console.error(
        `Error fetching El Mostrador article ${url}: ${message}${
  ...
  private parseContent(html: string, url: string): ScrapedArticleDetail {
    ...
    if (publishedDateString) {
      try {
        ...
        console.warn(
          `Could not parse date string "${publishedDateString}" for El Mostrador article ${url}: ${
            (e as Error).message
          }`
        );
      }
    ...
    if (mainContentContainer.length > 0) {
      console.log(
        `Extracting content using 'div.d-the-single-wrapper__text p' strategy for ${url}`
      );
      ...
      console.warn(
        `Main content container 'div.d-the-single-wrapper__text' not found for ${url}. Content might be missing.`
      );
  ...
  public async scrapeArticle(
    ...
    try {
      ...
      if (!articleDetail.content) {
        console.warn(
          `El Mostrador article content appears empty after parsing: ${url}`
        );
      }
      return articleDetail;
    } catch (error) {
      console.error(
        `Error parsing El Mostrador article content for ${url}:`,
        error
      );
      return null;
    }
